...
/** تصميم دائري: البوصلة بالمنتصف كعنصر رئيسي + اسم الصلاة القادمة وعدّادها تحتها. */
class PrayerWidgetCircleProvider : AppWidgetProvider() {
    ...
    fun updateIds(context: Context, mgr: AppWidgetManager, ids: IntArray) {
        ...
        for (id in ids) {
            val (bgW, bgH) = WidgetRenderer.currentSizePx(context, mgr, id, 110, 110)
            val qiblaPx = kotlin.math.min(bgW, (bgH * 0.75f).roundToInt())
            val chromeW = (bgW * 0.9f).roundToInt(); val chromeH = (bgH * 0.28f).roundToInt()

            val views = RemoteViews(context.packageName, R.layout.widget_prayer_circle)
            WidgetRenderer.fillBackground(context, views, R.id.widget_bg_image, R.id.widget_root, bgW, bgH)
            WidgetRenderer.fillQibla(context, views, loc, R.id.widget_qibla_image, null, qiblaPx)
            ...
            val openAppPending = PendingIntent.getActivity(
                context, 4320, Intent(context, MainActivity::class.java),
                PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE
            )
            views.setOnClickPendingIntent(R.id.widget_qibla_image, openAppPending)
